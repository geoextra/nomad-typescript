 ...
 * no description
 */
export class ScalingApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getScalingPolicies(region?: string, namespace?: string, index?: number, wait?: string, stale?: string, prefix?: string, xNomadToken?: string, perPage?: number, nextToken?: string, _options?: Configuration): Promise<RequestContext> {
     ...
     * @param nextToken Indicates where to start paging for queries that support pagination.
     */
    public async getScalingPolicy(policyID: string, region?: string, namespace?: string, index?: number, wait?: string, stale?: string, prefix?: string, xNomadToken?: string, perPage?: number, nextToken?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'policyID' is not null or undefined
        if (policyID === null || policyID === undefined) {
            throw new RequiredError("ScalingApi", "getScalingPolicy", "policyID");
        }
...
export class ScalingApiResponseProcessor {

    /**
     * Unwraps the actual response sent by the server from the response context and deserializes the response content
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getScalingPolicies
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getScalingPolicies(response: ResponseContext): Promise<Array<ScalingPolicyListStub> > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: Array<ScalingPolicyListStub> = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "Array<ScalingPolicyListStub>", ""
            ) as Array<ScalingPolicyListStub>;
            return body;
        }
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getScalingPolicy
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getScalingPolicy(response: ResponseContext): Promise<ScalingPolicy > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: ScalingPolicy = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "ScalingPolicy", ""
            ) as ScalingPolicy;
            return body;
